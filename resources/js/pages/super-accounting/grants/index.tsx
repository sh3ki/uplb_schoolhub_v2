...
export default function GrantsIndex({ tab, grants, recipients, students, schoolYears, currentSchoolYear, filters }: Props) {
    ...
    const handleTabChange = (value: string) => {
        setActiveTab(value);
        router.get('/super-accounting/grants', { tab: value }, { preserveState: true });
    };

    const handleFilter = () => {
        router.get('/super-accounting/grants', {
            tab: 'recipients',
    ...
    const handleReset = () => {
        ...
        setSchoolYear('all');
        setStatusFilter('all');
        router.get('/super-accounting/grants', { tab: 'recipients' });
    };

    const handleCreateGrant = (e: React.FormEvent) => {
        e.preventDefault();
        grantForm.post('/super-accounting/grants', {
            onSuccess: () => {
    ...
    const handleEditGrant = (e: React.FormEvent) => {
        e.preventDefault();
        if (editingGrant) {
            grantForm.put(`/super-accounting/grants/${editingGrant.id}`, {
                onSuccess: () => {
                    setIsGrantModalOpen(false);
    ...
    const handleDeleteGrant = (id: number) => {
        if (confirm('Are you sure you want to delete this grant?')) {
            router.delete(`/super-accounting/grants/${id}`);
        }
    };
    ...
    const handleAssign = (e: React.FormEvent) => {
        e.preventDefault();
        assignForm.post('/super-accounting/grants/recipients', {
            onSuccess: () => {
    ...
    const handleRemoveRecipient = (id: number) => {
        if (confirm('Are you sure you want to remove this recipient?')) {
            router.delete(`/super-accounting/grants/recipients/${id}`);
        }
    };
